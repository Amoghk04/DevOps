...
export default function TileGrid() {
  ...
  useEffect(() => {
    const { patterns, descriptions } = generateSequencePatterns();
    setTiles(patterns);
    ...
    const hidden = patterns.map((row, rowIndex) => {
      // Ensure we don't hide the first 2 tiles for Fibonacci sequences
      const minPosition = descriptions[rowIndex].includes('Fibonacci') ? 2 : 0;
      
      // Generate exactly one hidden position
      const pos = Math.floor(Math.random() * (6 - minPosition)) + minPosition;
      
      return [pos]; // Return array with single position
    });
                ...
                >
                  ...
                  {isHidden ? (
                    <input
                      ...
                      className={`relative z-10 w-10 h-10 text-center rounded-md bg-gray-800 border ${
                        isCorrect ? 'text-green-400 border-green-400' : 'text-gray-200 border-gray-600'
                      }`}
                      style={{ 
